# HydraDataHive — Pre-Alpha Progress Log

> **Historical record.** This file contains the development progress from the
> pre-alpha phase (commits `34558da` through `9c54cb8`). The current working
> document is [PROGRESS.md](./PROGRESS.md).
>
> See also: [NEW_DEPLOYMENT_TEST_GUIDE.md](./NEW_DEPLOYMENT_TEST_GUIDE.md) for
> the UX walkthrough, [RELEASE_NOTES_ALPHA1.md](./RELEASE_NOTES_ALPHA1.md) for
> the alpha release summary.

---

## 1. Status at a glance (pre-alpha)
...
All 12 build steps done, plus the cascade feature added in step 13:
...
| `app/services/sync.py`                             | Mesh pull + deletion cascade. `_peer_spec(ip)` looks up port in DB. |
...
## 6. Open items at end of pre-alpha

| # | Item                                                              | Status       |
| - | ----------------------------------------------------------------- | ------------ |
| 1 | Full 3-way mesh approved both ways (node1↔node2↔node3↔node1)        | ✅ Done      |
| 2 | File deletion cascade E2E                                         | ✅ Done      |
| 3 | Cleanup dead `sync-pending-hellos` CLI subcommand                 | ✅ Done      |
| 4 | Quick-start UX testing guide                                      | ✅ Done (`NEW_DEPLOYMENT_TEST_GUIDE.md`) |
| 5 | Dark-mode `text-muted` + DataTables contrast                      | ✅ Done      |
| 6 | Document screenshots                                              | ⏳ Deferred to alpha-tester feedback phase |
| 7 | `out/` backup feature                                             | ⏳ Deferred to v2 (PLAN.md §11) |
| 8 | TLS / mTLS                                                        | ⏳ Deferred to v2 |
| 9 | Self-IP rows in `/neighbors` (visual noise)                       | ⏳ Deferred — harmless |
| 10 | Cascade convergence via real tombstones (reject re-pulls of tombstoned SHA) | ⏳ Deferred to v2 |
